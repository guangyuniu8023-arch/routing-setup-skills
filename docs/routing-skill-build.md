...
- `type` — skill 的输出类型。常见值：`generate:image` / `generate:video` / `generate:text` / `clarify` / `search` / `analyze`，也可按项目自定义
- `allowed_actions` — 该 skill 可调用的 action 子集。从 `app/agent/plan_tools.py` 的 `_EXEC_TOOLS` 列表中选取适用项

**Body 部分（replan 指令）—— AI 生成草稿，用户审阅：**

AI 读取 frontmatter + Baseline Intent，自动生成 replan 指令草稿，至少包含以下内容（可按需增加其他段落如输入格式、输出格式、示例等）：
- 适用场景：skill 处理哪些情况
- 触发条件：何时启动 replan
- 执行规则：replan 中的约束
- 不适用声明：明确边界外的情况

用户审阅草稿后确认或修改，可增加项目特定的段落。AI 写入 SKILL.md body。
...
AI 从所有已构建的 SKILL.md frontmatter 自动提取信息，生成 Baseline Intent 表格（兼容路由质量配置模板）。每个 skill 一行，包含名称、意图、类型、allowed_actions。用户确认表格完整性即可。config.md 携带 Phase A 已确认的必须通过场景和不该走这里场景。路由质量调优在此基础上扩展（从 3-5 个扩到更多），而不是从零重建。
...
| Description | frontmatter `description` | Baseline Intent "一句话意图" | 意图导向，50-300 字符，含核心意图+边界条件 |
| Type | frontmatter `type` | generative 判断 | 常见值：`generate:image` / `generate:video` / `generate:text` / `clarify` / `search` / `analyze`，可自定义 |
...
description: ...          # 意图导向, 50-300 chars, 含核心意图+边界条件
type: generate:image      # 常见值：generate:image | generate:video | generate:text | clarify | search | analyze，可自定义
allowed_actions:
  - respond
  - generate_image
  - load_skill
...
|-----------|-----------|------|------------------|----------------|---------|----------|
| image-editing | 用户想对已有图片进行编辑修改 | generate:image | 1. "把背景换成海滩" 2. "去掉照片里的路人" 3. "把这张图调亮一点" | 1. "画一只猫"(generation) 2. "这张图是什么"(search) | 有参考图+创意描述 | image-generation |
```
