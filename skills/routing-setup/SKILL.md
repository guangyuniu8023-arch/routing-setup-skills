...
5. **Skill 清单**：让用户列出这个 Agent 需要的所有 skill（可以是初步想法，后续可调整）
...
类型字段填写 skill 的输出类型。常见值：`generate:image` / `generate:video` / `generate:text` / `clarify` / `search` / `analyze`，也可按项目自定义（如 `diagnose`、`recommend`）。
...
为项目生成路由测试 runner：
- 测试文件 `tests/routing_test.py`：接收 4-tuple，调用 planner，比较结果
- 测试命令记录到 `.routing-quality/config.md` 的 `test_command` 字段
- 成功标准：`actual_skill == expected_skill`，3-run 多数决消除 LLM 随机性

AI 生成测试 runner 骨架代码，用户确认。详细规范见 `docs/routing-setup.md`。
